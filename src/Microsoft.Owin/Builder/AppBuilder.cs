...
namespace Microsoft.Owin.Builder
{
    ...
    public class AppBuilder : IAppBuilder
    {
        ...
        //                      <firstParamType, delegate of construct, args>
        private readonly IList<Tuple<Type, Delegate, object[]>> _middleware;
        private readonly IDictionary<Tuple<Type, Type>, Delegate> _conversions;//<<the Delegate's ReturnType,the Delegate's FirstParamType>, the Delegate>  通过AddSignatureConversion
        ...
        {
            // middleware = UseHandlerMiddleware， 
            // args[0] = context =>
            // {
        ...
        private object BuildInternal(Type signature) //signature = Func<IDictionary<string, object>, Task>
        {
            object app;
            ...
            // 调用_middleware中的构造函数，生成对应Middlewave，再调用Middleware的Invoke方法，串成app
            foreach (var middleware in _middleware.Reverse())
            {
                Type neededSignature = middleware.Item1; // 构造函数第一参数
                Delegate middlewareDelegate = middleware.Item2; // 构造函数的委托
                object[] middlewareArgs = middleware.Item3; // 构造函数的其他参数

                app = Convert(neededSignature, app); // app is Func<IDictionary<string, object>, Task>
                object[] invokeParameters = new[] { app }.Concat(middlewareArgs).ToArray(); // 调用构造参数
                app = middlewareDelegate.DynamicInvoke(invokeParameters); // 调用构造函数, 生成Middleware, app is instance of Middleware, 将上次的app传进去，从而串在一起
                app = Convert(neededSignature, app); // app is Invoke method of Middleware
            }
        ...
        #region == 生成Middileware委托，但没有执行，放在_middleware中 ==
        // Tuple<Type, Delegate, object[]>
        //      第一个参数类型， 委托， 参数
        ...
        private static Tuple<Type, Delegate, object[]> ToMiddlewareFactory(object middlewareObject, object[] args)
        {
            ...
            // 调用Constants.Invoke方法 public AppFunc Invoke
